...
async function run() {
  await test([{ chat_id: 6513777, text: "Test" }]);
  await test({ message: { chat_id: 6513777, text: "Test" } });
  await test({ messages: [{ chat_id: 6513777, text: "Test" }] });
  await test({ chat_id: "6513777", text: "Test" }); // Just in case, as a string
}
run();
